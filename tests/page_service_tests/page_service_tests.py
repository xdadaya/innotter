...
def test_accept_all_follow_requests(user_factory: UserFactory, page_factory: PageFactory) -> None:
    page = page_factory.create(is_private=True)
    faker = Faker()
    user1 = user_factory.create(username=faker.name(), email=faker.email())
    user2 = user_factory.create(username=faker.name(), email=faker.email())
    user3 = user_factory.create(username=faker.name(), email=faker.email())
    PageService.follow(user1, page.id)
    PageService.follow(user2, page.id)
    PageService.follow(user3, page.id)
...
def test_accept_single_follow_requests(user_factory: UserFactory, page_factory: PageFactory) -> None:
    ...
    PageService.follow(user3, page.id)
    assert FollowRequest.objects.filter(page=page).count() == 3
    PageService.accept_single_request(page.id, FollowRequest.objects.get(follower=user2).id)
    assert FollowRequest.objects.filter(page=page).count() == 2
    assert Page.objects.get(id=page.id).followers.count() == 1
    assert Page.objects.filter(id=page.id, followers__id__exact=user2.id).count() == 1
...
def test_accept_single_follow_requests(user_factory: UserFactory, page_factory: PageFactory) -> None:
    ...
    PageService.follow(user3, page.id)
    assert FollowRequest.objects.filter(page=page).count() == 3
    PageService.reject_single_request(page.id, FollowRequest.objects.get(follower=user2).id)
    assert FollowRequest.objects.filter(page=page).count() == 2
    assert Page.objects.get(id=page.id).followers.count() == 0
    assert Page.objects.filter(id=page.id, followers__id__exact=user2.id).count() == 0
